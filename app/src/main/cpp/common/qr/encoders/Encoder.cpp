...
namespace silgrid {
	...
	void Encoder::init(std::shared_ptr<ResourceLoader> resourceLoader) {
	...
	bool Encoder::canProcess(std::string& input) {
		return regex_match(input, stringValidationRegex);
	}

	const std::string& Encoder::getName() const {
		return name;
	}

	const std::shared_ptr<ErrorCorrector> Encoder::getErrorCorrector() const {
		return errorCorrector;
	}

	void Encoder::setErrorCorrector(std::shared_ptr<ErrorCorrector> errorCorrector) {
	...
	int **Encoder::encode(std::string& input) {
		auto size = std::bitset<16>(input.size()).to_string();
		...
		result += std::string("0000").substr(0, requiredBitSize - result.size() >= 4 ? 4 :
												requiredBitSize - result.size());

		if (result.size() % 8 != 0) {
			result += std::string("00000000").substr(result.size() % 8);
		}
	...
	int **Encoder::generateMatrix(std::string& code) {
		...
		for (int i = 0; i < alignmentPatternPositions.size(); ++i) {
			for (int j = 0; j < alignmentPatternPositions.size(); ++j) {
				...
				if ((alignmentPatternPositions[i] < 10 && alignmentPatternPositions[j] < 10)
					|| (alignmentPatternPositions[i] < 10 &&
						alignmentPatternPositions[j] > version.getBarcodeSize() - 10)
					|| (alignmentPatternPositions[j] < 10 &&
						alignmentPatternPositions[i] > version.getBarcodeSize() - 10)) {
					continue;
				}

				for (int k = alignmentPatternPositions[i] - 2;
					 k < alignmentPatternPositions[i] + 3; ++k) {
					matrix[k][alignmentPatternPositions[j] - 2] = blackCell;
					matrix[k][alignmentPatternPositions[j] + 2] = blackCell;
				}
				for (int k = alignmentPatternPositions[j] - 2;
					 k < alignmentPatternPositions[j] + 3; ++k) {
					matrix[alignmentPatternPositions[i] - 2][k] = blackCell;
					matrix[alignmentPatternPositions[i] + 2][k] = blackCell;
				}

				for (int k = alignmentPatternPositions[i] - 1;
					 k < alignmentPatternPositions[i] + 2; ++k) {
					matrix[k][alignmentPatternPositions[j] - 1] = whiteCell;
					matrix[k][alignmentPatternPositions[j] + 1] = whiteCell;
				}
				for (int k = alignmentPatternPositions[j] - 1;
					 k < alignmentPatternPositions[j] + 2; ++k) {
	...
	int **Encoder::addCode(int **matrix, std::string& code) {
	...
	int Encoder::calculatePenalty(int **matrix) {
		...
		float darkPercent =
				((float) darkModules / (version.getBarcodeSize() * version.getBarcodeSize())) *
				100.0f;
		...
		penalty += std::min(prevMultiple, nextMultiple) * 10;
	...
	int **Encoder::createMaskedMatrix(const int **codeMatrix, const int **fullMatrix) {
		...
		int penalty = -1;
		auto maskers = MaskersFactory::getMatrixMaskers(version.getBarcodeSize(), fullMatrix,
														codeMatrix);
	...
	void Encoder::addVersionInfo(int **matrix) {
		std::string value = std::bitset<2>(
				(unsigned long long int) errorCorrector->getLevelValue()).to_string();
		value += std::bitset<3>((unsigned long long int) masker->getNumber()).to_string();
		auto formatString = generateInfoString(15, value, std::string("10100110111"));

		for (int i = 0; i < 8; ++i) {
			if (matrix[i][8] == -1) {
				matrix[i][8] =
						formatString[formatString.size() - i - 1] == '1' ? blackCell : whiteCell;
				matrix[8][i] = formatString[i] == '1' ? blackCell : whiteCell;

			}
			matrix[8][version.getBarcodeSize() - 1 - i] =
					formatString[formatString.size() - i - 1] == '1' ? blackCell : whiteCell;
			matrix[version.getBarcodeSize() - 1 - i][8] =
					formatString[i] == '1' ? blackCell : whiteCell;
		}
		matrix[version.getBarcodeSize() - 8][8] = blackCell;
		matrix[8][8] = formatString[7] == '1' ? blackCell : whiteCell;

		if (version.getVersionNumber() >= 7) {
			auto versionString = generateInfoString(18, std::bitset<6>(
					(unsigned long long int) version.getVersionNumber()).to_string(),
													std::string("1111100100101"));

			for (int i = 0; i < 6; ++i) {
				for (int j = 0; j < 3; ++j) {
					matrix[version.getBarcodeSize() - 11 + j][i] =
							versionString[i * 3 + j] == '1' ? blackCell : whiteCell;
					matrix[i][version.getBarcodeSize() - 11 + j] =
							versionString[i * 3 + j] == '1' ? blackCell : whiteCell;
				}
			}
		}
	...
	std::string Encoder::generateInfoString(int length, std::string infoValue,
											std::string baseGenerator) {
		std::string zeros("000000000000000");
		std::string divisor = infoValue + zeros.substr(0, length - infoValue.size());
		...
		while (divisor.size() > length - infoValue.size()) {
			...
			divisor = std::bitset<18>((unsigned long long int) res).to_string();
			divisor = divisor.substr(divisor.find('1'));
		}
		...
		if (length < 18) {
			long masked =
					strtol(divisor.c_str(), nullptr, 2) ^strtol("101010000010010", nullptr, 2);
			return std::bitset<15>((unsigned long long int) masked).to_string();
		} else {
			return divisor;
		}
	}

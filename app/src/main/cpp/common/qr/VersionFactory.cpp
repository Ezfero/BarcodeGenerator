...
namespace silgrid {
	...
	void VersionFactory::init(std::shared_ptr<ResourceLoader> resourceLoader) {
	...
	std::shared_ptr<Version> VersionFactory::getVersion(const Encoder encoder,
		...
		return std::shared_ptr<Version>();
	}

	std::shared_ptr<Version> VersionFactory::find(const int length, const Encoder& encoder,
												  const VersionInfo& info) {
		if (encoder.getName().compare(NumericEncoder::NAME) == 0) {
			if (length <= info.getNumericModeSymbols()) {
				return std::make_shared<Version>(encoder, info);
			}

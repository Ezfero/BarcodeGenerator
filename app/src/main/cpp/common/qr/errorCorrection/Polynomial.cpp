...
namespace silgrid {
	...
	Polynomial operator+(const Polynomial& lhs, const Polynomial& rhs) {
		int minDegree = std::min(lhs.degree, rhs.degree);
		std::vector<int> sumParams(lhs.params);
		for (int i = 0; i <= minDegree; ++i) {
			if (lhs.mode == Polynomial::Mode::GALOIS) {
				if (lhs.params[i] != lhs.getNeutralElement() &&
					rhs.params[i] != rhs.getNeutralElement()) {
...
//	}
	...
	Polynomial operator*(const Polynomial& lhs, const Polynomial& rhs) {
		...
		auto params = std::vector<int>((unsigned long) length);

		if (lhs.mode == Polynomial::Mode::GALOIS) {
			std::vector<std::vector<int>> multipliers((unsigned long) length);

			for (int i = 0; i <= lhs.degree; ++i) {
				for (int j = 0; j <= rhs.degree; ++j) {
					if (lhs.params[i] != lhs.getNeutralElement() &&
						rhs.params[j] != lhs.getNeutralElement()) {
	...
	void Polynomial::toMode(Polynomial::Mode mode) {
		...
		if (mode == Mode::NORMAL) {
			for (int i = 0; i < params.size(); ++i) {
				params[i] = params[i] == getNeutralElement() ? normalNeutralElement
															 : logAntilogTable.getValue(params[i]);
			}
		} else {
			for (int i = 0; i < params.size(); ++i) {
				params[i] = params[i] == getNeutralElement() ? galoisNeutralElement
															 : logAntilogTable.getDegree(params[i]);
			}
		}
		this->mode = mode;
	}

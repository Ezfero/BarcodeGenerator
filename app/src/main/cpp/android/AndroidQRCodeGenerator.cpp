...
namespace silgrid {
	...
	std::shared_ptr<ResourceLoader> AndroidQRCodeGenerator::getResourceLoader() {
		if (!assetLoader) {
			assetLoader = std::make_shared<AssetLoader>(jniEnv, assetManager);
		}
		return assetLoader;
	}
	...
	void *AndroidQRCodeGenerator::createCodeImage(int matrixSize, int **matrix) {
		...
		jobject bitmap = jniEnv->CallStaticObjectMethod(bitmapClass, createBitmapMethod, matrixSize,
														matrixSize, config);
		...
		jniEnv->CallVoidMethod(bitmap, setPixelsMethod, pixels, 0, matrixSize, 0, 0, matrixSize,
							   matrixSize);
		...
		bitmap = jniEnv->CallStaticObjectMethod(bitmapClass, createScaledBitmapMethod, bitmap,
												matrixSize * 30, matrixSize * 30, false);
